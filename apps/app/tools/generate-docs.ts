...
function emitComponent(component: XuiComponent, story: StoryMeta | undefined): Emitted {
  ...
  // Two modules, and the split is what keeps the server bundle small: a preview imports the package
  // it demonstrates, so a page's data and its demo together drag the whole library — plus echarts
  // and konva — into a worker that renders neither. The data module is strings, and the previews
  // sit behind `previews.ts`, which only the browser entry point reaches.
  const lines: string[] = [
  ...
  lines.push(
    ...
    ...examples.map(example =>
      [
        ...
        // The class, by name rather than by reference — resolving it is the browser's job.
        ...(canPreview(example, example.template, localSelectors)
          ? [`      previewName: ${json(`Preview${example.name}`)}`]
          : []),
        '    },'
      ].join('\n')
    ),
  ...
  if (perExample.length > 0) {
    writeFileSync(join(outDir, 'components', `${slug}.preview.ts`), previewLines.join('\n'), 'utf8');
  }
...
writeFileSync(
  join(outDir, 'previews.ts'),
  [
    '// Generated by apps/app/tools/generate-docs.ts. Do not edit.',
    `import type { Type } from '@angular/core';`,
    '',
    '/**',
    ' * The demo components, one lazy chunk per package.',
    ' *',
    ' * Imported by the browser entry point and nothing else. A preview pulls in the package it',
    ' * demonstrates, and it only ever renders after hydration, so this is the seam that keeps the',
    ' * whole library — echarts and konva included — out of the server bundle.',
    ' */',
    'export const PREVIEWS: Record<string, () => Promise<Record<string, Type<unknown>>>> = {',
    ...emitted
      .filter(entry => entry.previews > 0)
      .map(
        entry => `  ${JSON.stringify(entry.summary.slug)}: () => import('./components/${entry.summary.slug}.preview'),`
      ),
    '};',
    ''
  ].join('\n'),
