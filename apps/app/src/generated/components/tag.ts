  ...
  template: `
    <div class="flex flex-wrap items-center gap-2">
      @for (t of tags; track t) {
        <xui-tag round large intent="primary" minimal removable (removed)="tags = tags.filter(x => x !== t)">{{
          t
        }}</xui-tag>
      }
    </div>
  ...
  symbols: [
    {
      ...
      name: 'XuiTag',
      selector: 'xui-tag',
      docs: 'A compact label chip — richer than the static `xui-badge`. Optionally shows a\nleading icon, reads as interactive, and can carry a remove ✕ that emits\n`removed`.',
      inputs: [
        {
          name: 'class',
          type: 'ClassValue',
