    ...
    @if (showPanelHeader()) {
      <header class="border-border flex h-11 shrink-0 items-center border-b px-1">
        @if (depth() > 1) {
          <button
            type="button"
            class="text-foreground-muted hover:bg-hover-overlay hover:text-foreground flex h-9 items-center gap-0.5 rounded-md pr-3 pl-1.5 text-sm focus-visible:outline-5 focus-visible:outline-offset-2"
            (click)="closePanel()"
          >
