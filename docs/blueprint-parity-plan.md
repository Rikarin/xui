...
### Phase 4 — Forms (14 packages) — 🚧 11 done (textarea, switch, radio, numeric-input, file-input, control-group, html-select, segmented-control, checkbox, input/InputGroup, form-field)
...
8. Phase 4 — forms (14 packages): ~~`textarea` → `switch` → `radio` → `numeric-input` → `file-input`
   ...
   (up) → `form-field` (up)~~ ✅ (11 done). Left: `slider` (**NEW** `@xui/slider` + `@xui/core/slider`
   — headless drag/keyboard/tick math; `min`/`max`/`stepSize`/`labelStepSize`/`labelRenderer`,
   vertical, `intent` track fill; the one substantial build left). The select family (Phase 6) is
   where forms meet overlays again.
