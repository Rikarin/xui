...
8. Phase 4 — forms (14 packages): ~~`textarea` → `switch` → `radio` → `numeric-input` → `file-input`
   ...
   COMPLETE (17 components).** Next: Phase 5 (data display & navigation — `tabs`, `tag`, `tree`,
   `table` up, `hotkeys`, `resize-sensor`) or Phase 6 (select family, which now has its `tag-input`
   dependency in place for `MultiSelect`). The select family is where forms meet overlays again.
