...
### Phase 8 — Advanced data grid (in scope; large) — 🚧 core + virtualized grid + interactions shipped (8.1 ✅, 8.2/8.3 ✅, 8.4 ✅, 8.5 ✅; frozen quadrants / editable cells / column virtualization deferred)
...
12. Phase 8 — advanced data grid: **8.1 `@xui/core/grid`** (region/size/locator model + `regionBounds`,
    ...
    `[xuiDataCell]` templates, `role=grid` + `aria-selected`/`aria-multiselectable` a11y — all
    browser-verified (21 data-table tests). 🚧 **Remaining (deferred within Phase 8):** column
    virtualization + the frozen quadrant renderer (`numFrozenRows`/`numFrozenColumns`), editable cells
    (`EditableCell`/`JSONFormat`/`TruncatedFormat`/`RowHeaderCell`), `onCompleteRender`, header menus,
    `ghostCellCount`. Then Phase 9 (labs — mostly skippable; a `xuiSlot` content-merge directive is the
    one candidate).
