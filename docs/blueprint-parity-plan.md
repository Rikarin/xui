...
10. Phase 6 — select family: ~~`@xui/core/query` (headless engine) → `select` → `multi-select` →
    ...
    Next: Phase 7 (date & time — `@xui/core/date-time` adapters already exist; build the calendar /
    date-input / date-range / time picker on top), then Phase 8 (data grid) and Phase 9 (labs).
