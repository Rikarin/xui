...
### Phase 6 — Select family (5 packages, depends on Phases 3+4) — 🚧 3 done (core/query, select, multi-select). Left: suggest, omnibar
...
9. Phase 5 — data display & navigation: ~~`tabs` (**NEW**) → `table` (up) → `resize-sensor` (core,
   ...
   (6 packages).** Next: Phase 6 (select family — `@xui/core/query` headless, then `select` /
   `multi-select` / `suggest` / `omnibar`), which now has its `tag-input` dependency in place for
   `MultiSelect`, and composes the Phase 3 popover with the Phase 4 form controls. The select family
   is where forms meet overlays again.
